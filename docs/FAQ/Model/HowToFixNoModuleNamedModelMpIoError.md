# How to Fix "No module named 'model_mp_io'" Error


## Problem Description

An error message appears when using the model training and inference library in Python's block-based mode.


## Examples of the Phenomenon

![img](https://mindplus.dfrobot.com.cn/mp2/FAQ/AITools/img/no_module_named.png)

![img](https://mindplus.dfrobot.com.cn/mp2/FAQ/AITools/img/mo_matching_distribution_found_for_model_mp.png)


## Solution

* First, confirm that the program is connected to the UNIHIKER M10 and running (Mind+ top menu: Connect 10.1.2.3).
* Next, make sure the UNIHIKER M10 system firmware is up to date (you can check the system version by long-pressing the Home menu > View System Information); it must be at least v0.4.1. To update the UNIHIKER system firmware: [Click here to view]()
